...
export default function WritingDashboard() {
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <h1 className="text-3xl font-bold mb-8">Your Writing Journey</h1>
      ...
      {/* Writing Stats and Achievements */}
      ...
      <Tabs defaultValue="all-rewards" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="all-rewards">All Writing Tools</TabsTrigger>
          <TabsTrigger value="current-rewards">Your Writing Tools</TabsTrigger>
        </TabsList>
  ...
  <Card className="shadow-xl bg-white rounded-lg border border-gray-200 p-6">
    <CardHeader>
      <CardTitle className="text-2xl font-semibold text-gray-800">Your Writing Tools</CardTitle>
    ...
    <CardContent>
      <div className="space-y-4">
        {levelRewards
          ...
          .map((reward) => (
            <div key={reward.level} className="border rounded-lg p-4 hover:shadow-lg hover:bg-gray-50 transition duration-300">
              <div className="flex items-center mb-3">
                <div className="bg-primary text-primary-foreground w-10 h-10 rounded-full flex items-center justify-center mr-4 shadow-md">
                  {reward.level}
                </div>
                <h3 className="font-semibold text-gray-800 text-lg">{reward.name}</h3>
                {reward.level === userData.level && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3">
                {reward.rewards.map((item, index) => (
                  <div
                    key={index}
                    className="flex items-center p-3 rounded-md bg-primary/10 text-gray-700 hover:bg-primary/20 transition-all duration-300 cursor-pointer"
                  >
        ...
        {userData.level < 10 && (
          <div className="mt-6 p-4 border border-dashed rounded-lg bg-muted/30">
            <h3 className="font-semibold flex items-center text-lg text-gray-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3">
              {nextLevelDetails?.rewards.map((item, index) => (
                <div
                  key={index}
                  className="flex items-center p-3 rounded-md bg-muted/50 text-muted-foreground hover:bg-muted/60 transition-all duration-300 cursor-pointer"
                >
                  <Gift className="h-5 w-5 text-muted-foreground mr-3" />
                  <span className="text-sm">{item}</span>
                </div>
              ))}
            </div>
